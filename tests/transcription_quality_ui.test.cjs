...
function setup({getUserMedia,fetcher,saved=''}={}) {
  ...
  vm.runInNewContext(source,{window,document:{getElementById:element},location:{href:'http://localhost/transcription_test.php'},URL:url,MediaRecorder:Recorder,
    navigator:{mediaDevices:{getUserMedia:getUserMedia||async function(){return {getTracks:()=>[{stop(){stops++;}}]};}},clipboard:{writeText:async()=>{}}},
  ...
  return {element,requests,windows,timers,get stops(){return stops;},get media(){return media;},
