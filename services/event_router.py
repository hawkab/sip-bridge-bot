...
async def handle_cdr_group_notification(delivery: DeliveryHub, event_store: EventStoreClient, rows: list[dict]) -> None:
    ...
    view_url = await _save_call_event(event_store, event.rows, attachment_path, attachment_name)
    await delivery.notify_event(
        ...
        parse_mode="Markdown",
        email_text=_append_event_link(msg, view_url, "Карточка звонка"),
        email_attachment_path=None if view_url else attachment_path,
        email_attachment_name=None if view_url else attachment_name,
    )
