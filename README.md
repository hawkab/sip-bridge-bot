...
Производительность: [пул HTTP-соединений, опрос очередей и память Python](docs/python-performance.md).
...
Отправка СМС доступна из веб-интерфейса, Telegram и email: [настройка и команды](docs/sms-sending.md).
...
| `TG_DEFAULT_SIM`   |  нет | `1`                          | Устаревший информационный параметр; для `/sms` SIM выбирается явно |
...
>     host port login password
>
> Если логин/пароль содержат спецсимволы вроде `@`, `:`, `/`, `#`, `?` или пробелы,
> используйте полный URL-формат с percent-encoding, например:
> `http://user:p%40ss%3Aword@host:port`
...
3. Входящие SMS со шлюза TG:
   ...
   - Для отправки используйте `/sms <SIM> <номер получателя> <текст>`; `/sms_ports` показывает доступных отправителей.
...
### Голосовое сообщение звонком

Веб-раздел «Голосовой вызов» и команда Telegram `/voicecall` позволяют надиктовать запись, позвонить сразу или назначить время. [Инструкция и установка](docs/voice-calls.md).

Номера принимающих SIM, ответы из карточек и маршруты голосовых вызовов: [инструкция](docs/sim-and-chats.md).
